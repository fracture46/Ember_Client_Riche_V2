...
export default Route.extend({
  ...
  actions:{
    addTo:function(source, dest, items){
      let model=this.modelFor(this.routeName); // correspond à une instance de Liste
    ...
    removeFrom:function(source, dest, items){
    ...
    dblClick:function(){
      let model=this.modelFor(this.routeName);
      let objet = Ember.get(model,'selectedDispoItemsIds');
      let dest = Ember.get(model, 'includedItems');
      let source = Ember.get(model, 'dispoItems');
      debugger
      dest.pushObjects(objet);
      source.removeObjects(objet);
      Ember.set(model,'selectedDispoItemsIds',[]);
      Ember.set(model,'selectedIncludedItemsIds',[]);
    }
  }
